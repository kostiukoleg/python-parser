...
$company = NULL;
$this->breadcrumbs = [Yii::t("site", "Услуги")];
$this->pageTitle = Yii::t('CompanyModule.company', 'Строительные услуги: проверенные компании и мастера, цены и отзывы');
$this->description = Yii::t('site','Строительные услуги: большой каталог компаний и мастеров. Строительные, инженерные и отделочные работы. Капитальное строительство и ремонт | портал ДомСтрой.');
$this->keywords = Yii::t('site','строительные услуги');
if (Yii::app()->getRequest()->getParam('company') !== null) {
    ...
    if ($company) {
        ...
        $this->pageTitle = 'Все услуги компании '.$company->name.(($city) ? ' ('.$city->title.')' : '').' - Строительный портал ДомСтрой';
        $this->description = 'Весь ассортимент услуг '.$company->name.(($city) ? ' ('.$city->title.')' : '').' для ремонта и строительства. Выбирайте лучшее!';
        $this->keywords = 'Услуги компании ' . $company->name;
        $this->canonicalLink = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].'/service?company='.Yii::app()->getRequest()->getParam('company');
    }
} else {
    $this->canonicalLink = $_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].'/service';
}
...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
            <header>
                ...
                <?php
                    if ($company) {
                        echo 'Услуги компании ' . $company->name;
                    } else {
                        echo Yii::t("site", "Услуги");
                    }
                ?>
                </h1>
            </header>
    ...
    <div class="row">
        <aside class="col-sm-3">
            <?php if ($company) { ?>
                ...
                <div id="add-toggle" class="add-form" style="text-align: center;">
                    <span style="font-size:18px; color:#244A64;font-weight: bold;">Уточнить поиск услуг компании</span></br></br><span style="font-size:18px; color:#244A64;"><?php echo $company->name; ?></span>
                    <?php if($cat == 1){ ?>
            ...
            <div id="add-toggle" class="collapse in add-form">
                ...
                <div>
                    <?php $this->widget('application.modules.store.widgets.filters.CategoryFilterWidget',['parent'=>[6,7]]); ?>
                </div>
                <div>
		...
		<?php } ?>       
                <div class="add-img">
                    <?php $this->widget('application.modules.banner.widgets.bannerWidget',['position'=>14])?>
                    <?php $this->widget('application.modules.banner.widgets.bannerWidget',['position'=>15])?>
                </div>
            </div>
         <?php } ?>
