 ...
 *
 * @property UserBalanceOperation $balance_operations
 */
...
class User extends yupe\models\YModel
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            ['nick_name, first_name, last_name, email, hash', 'required'],
            ...
            [
                'id, update_time, create_time, middle_name, first_name, last_name, nick_name, email, gender, avatar, status, access_level, visit_time, phone, items',
                'safe',
                'on' => 'search'
            ],
            ...
            [
                'phone',
                'match',
                'pattern' => Yii::app()->params['phone-pattern'],
    ...
    public function getNickName($id = null)
    {
        $user = self::findByPk($id);
    ...
    public function getCompany(){
        $company = Company::model()->find(['condition'=>'owner_id = '.$this->id]);
        if($company === null){
            return false;
        }else{
            return $company;
    ...
    /*Вытягиваем данные о менеджере компании*/
    public function getManagerToCompany($id){
        $manager = Yii::app()->getDb()->createCommand()
    ...
    public function getPlanTo($id){
        $at = array();
        $at = Yii::app()->getDb()->createCommand()
            ->select('plan_to')
            ->from('{{company}}')
            ->where('id = :id', [':id' => $id])
            ->queryAll();
    ...
    public function getPlanName($id){
        ...
        $at = Yii::app()->getDb()->createCommand()
            ...
            ->where('id = :id', [':id' => $id])
            ->queryAll();
        $bt = Yii::app()->getDb()->createCommand()
            ->select('name')
            ->from('{{store_plan}}')
            ->where('id = :id', [':id' => $at[0]['plan']])
            ->queryAll();
        if (!empty($at[0]['plan'])) {
            return $bt[0]['name'];
        }
    ...
    public function recoveryPasswordUser($mail,$hash){
        $connection = Yii::app()->db;
        $command=$connection->createCommand('UPDATE `main_user_user` SET `hash` ="'.$hash.'" WHERE `email`="'.$mail.'" ');
        $command->execute();
    }
    public function getCompanyToID($id_user){
        //$at = array();
        $at = Yii::app()->getDb()->createCommand()
            ->select('id')
            ->from('{{company}}')
            ->where('owner_id = :id_user', [':id_user' => $id_user])
            ->queryAll();
        if (isset($at[0])) {
            return $at[0]['id'];
        }
    ...
    public function getPublished($id){
        $published = Yii::app()->getDb()->createCommand()
            ->select('date, amount')
            ->from('{{published}}')
            ->where('id_company = :id', [':id' => $id])
            ->queryAll();
        return $published[0];
    }
}
