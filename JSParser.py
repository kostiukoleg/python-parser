...
#залогінюємось на сайті
    def login(self, login_link):
        try:
            ...
            login_data = {
                "passno": "Zm9yNzUxMSU0MA==",
                "idsaveyn": "Y",
                "id": "7552",
                "returnUrl": "jh2ISr6qitW90ERi3vig+Xa+/2AbjWqZ/X5jbcCGxss="
            }
...
#парсимо всі ссилки на авто по сторінкам
    def get_pages(self, page_start=1, page_end=2):
        for i in range(page_start,page_end+1):
            unf = uniform(1,4)
            time.sleep(unf)
            print("Page %d. Sleep %d." % (i, unf))
            link = '{}?&ac={}&acc={}&atn={}&page1=1&page={}'.format(config['GLOVIS']['URL'], urllib.parse.quote_plus("TQhYt3GD6GvgPdVw1QX+Wg=="), urllib.parse.quote_plus("u9cesU3il5ljSzAzHZzmEg=="), urllib.parse.quote_plus("6/6K8Btil9HcmO4rJnpO4w=="), str(i))
...
#складаємо всі ссилки на авто в один файл
    def get_all_links(self, car_link=config['GLOVIS']['CAR_LINK'], file_name="car_id3.txt"):
        ...
        return list(map(lambda x: '{}?&rc={}&gn={}&ac={}&atn={}&acc={}'.format(car_link, x[0], x[1], urllib.parse.quote_plus("TQhYt3GD6GvgPdVw1QX+Wg=="), urllib.parse.quote_plus("6/6K8Btil9HcmO4rJnpO4w=="), urllib.parse.quote_plus("u9cesU3il5ljSzAzHZzmEg==")), res))
        ...
        if(html != ''):
            try:
                if(len(html.find("p.carnm"))):
                    return self.clear_car_name(self.ko_translate(html.find("p.carnm")[0].text, "en"))
                else:
                    return ""
            except Exception as e:
            ...
            try:
                if(len(html.find("p.carnm"))):
                    ...
                    if(res):
                        res = res.group(1)
                        category = res[1:-1]
                        if(category != "Kia" and category != "Hyundai" and category != "Ssangyong" and category != "Genesis"):
                            print(category)
                        category = re.sub(" Samsung", "", category)
                        category = re.sub("ChevroletDaewoo", "Chevrolet", category)
                        category = re.sub(" Daewoo", "", category)
    ...
    def get_car_mark(self, html):
        if(html != ''):
            ...
            try:
                ...
                print('Can\'t connect to MYSQL. Reason %s.' % e)
            try:
                if(len(html.find("p.carnm"))):
...
#витягуємо всі данні на авто
    def get_car(self, link):
        ...
        car['title'] = "" if not lot_number and not self.get_car_title(html) else lot_number+" "+self.get_car_title(html)
