  ...
  function __construct() {
        ...
        foreach($mails as $mail){
          if(preg_match('/^[-._a-zA-Z0-9]+@(?:[a-zA-Z0-9][-a-zA-Z0-9]+\.)+[a-zA-Z]{2,6}$/', $mail)){
            Mailer::sendMimeMail(array(
              ...
              'emailFrom' => 'kostiukoleg@rambler.ru',
              'nameTo' => $sitename,
              'emailTo' => $mail,
    ...
    if(isset($_POST['send2'])){
      ...
      if(!$error){
        ...
        $sitename = MG::getSetting('sitename');       
        //$message = str_replace('№', '#', $feedBack->getMessage());        
        $body = MG::layoutManager('email_feedback2', array('phone'=>$feedBack->getPhone(), 'name2'=>$feedBack->getFio(), 'email2'=>$feedBack->getEmail(), 'text2'=>$feedBack->getMessage()));
