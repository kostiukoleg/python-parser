    ...
    try:
        ...
        for item in items:
            if item.has_attr('src'):
                ...
                if match:
                    images.append(item.get('src').replace('_S', ''))
                else: 
                    continue
        return images
    except Exception as e:
...
def get_missed_car_id(html):
    if(html != ''):
        ...
        for item in items:
            try:
                lot_id = ko_translate(rm_new_line(str(item.find("div").find("strong", class_="i_comm_main_txt").text.strip())), "en")
            except Exception as e:
            ...
            with db_connection.cursor() as cursor:
                ...
                if(len(result) == 0):
                    print("Car lot id:" ,lot_id, "Car name:", car_name)
...
def get_car(link):
    ...
    try:
        if(isinstance(html, int)):
            ...
            if((year < 2012) or category != "Kia Motors" and category != "Genesis" and category != "Hyundai" and category != "Renault" and category != "Chevrolet"):
                return
